...
const PoolSpill: React.FC<Props> = ({needHelp, setNeedHelp, multiplayer}) => {
    const userInfo = useSelector(selectUser);

    const [currentSelection, setCurrentSelection] = useState<number>(0);
    const [flowingSpeed] = useState<number>(Math.floor((Math.random() * 5) + 2));
    const [time] = useState<number>(Math.floor((Math.random() * 10) + 5));
    ...
    return (
        <form className="level__age level__container">
            ...
            <div className="level__container__task">
                <strong>Water flows out of the pool at a rate of {flowingSpeed} {flowingSpeed === 1 ? "liter" : "liters"} per minute. What is the pool capacity if it takes {time} minutes to get empty?</strong>
            </div>
