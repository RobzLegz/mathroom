...
const LevelOption: React.FC<Props> = ({task}) => {
    ...
    if(userInfo.loggedIn){
        return (
            <div
                className={`levels__container__tasks__container__level levels__container__tasks__container__level__${Number(task.level) > 30 ? "purple" : Number(task.level) > 20 ? "red" : Number(task.level) > 10 ? "yellow" : "green"}`}
